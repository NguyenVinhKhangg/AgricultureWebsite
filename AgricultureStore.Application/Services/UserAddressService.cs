...
namespace AgricultureStore.Application.Services
{
    public class UserAddressService : IUserAddressService
    {
        ...
        {
            _logger.LogDebug("Getting addresses for user: {UserId}", userId);
        ...
        {
            _logger.LogDebug("Getting address with ID: {AddressId}", id);
        ...
        {
            _logger.LogDebug("Getting default address for user: {UserId}", userId);
        ...
        public async Task<UserAddressDto> CreateAddressAsync(int userId, CreateUserAddressDto createDto)
        {
            ...
            _logger.LogInformation("Address created - AddressId: {AddressId}, UserId: {UserId}", address.AddressId, userId);

            if (createDto.IsDefault)
            {
                await _unitOfWork.UserAddresses.SetDefaultAddressAsync(userId, address.AddressId);
                await _unitOfWork.SaveChangesAsync();
                _logger.LogInformation("Address {AddressId} set as default for user {UserId}", address.AddressId, userId);
            }
        ...
        {
            _logger.LogDebug("Updating address with ID: {AddressId}", id);
            ...
            _logger.LogInformation("Address updated - AddressId: {AddressId}", id);

            if (updateDto.IsDefault == true)
            {
                await _unitOfWork.UserAddresses.SetDefaultAddressAsync(address.UserId, id);
                await _unitOfWork.SaveChangesAsync();
                _logger.LogInformation("Address {AddressId} set as default", id);
            }
        ...
        {
            _logger.LogDebug("Deleting address with ID: {AddressId}", id);

            await _unitOfWork.UserAddresses.DeleteAsync(id);
            await _unitOfWork.SaveChangesAsync();

            _logger.LogInformation("Address deleted - AddressId: {AddressId}", id);
            return true;
        }

        public async Task<bool> SetDefaultAddressAsync(int userId, int addressId)
        {
            _logger.LogDebug("Setting default address {AddressId} for user {UserId}", addressId, userId);
            ...
            {
                _logger.LogWarning("Failed to set default address - UserId: {UserId}, AddressId: {AddressId}", userId, addressId);
            }
